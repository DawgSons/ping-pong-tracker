...
CREATE OR REPLACE FUNCTION "public"."nearby_tables"("lat" double precision, "lng" double precision, "radius_meters" double precision) RETURNS TABLE("id" "uuid", "location" "public"."geography", "image_url" "text", "description" "text", "condition" "text", "distance_meters" double precision)
    LANGUAGE "plpgsql"
    AS $$
begin
  return query
...
ALTER FUNCTION "public"."nearby_tables"("lat" double precision, "lng" double precision, "radius_meters" double precision) OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "api"."tables" (
...
ALTER TABLE "api"."tables" OWNER TO "postgres";


COMMENT ON TABLE "api"."tables" IS 'storage for ping pong table info';
...
CREATE TABLE IF NOT EXISTS "public"."tables" (
    ...
    "added_by" "uuid",
    "metadata" "jsonb",
    CONSTRAINT "tables_condition_check" CHECK (("condition" = ANY (ARRAY['good'::"text", 'fair'::"text", 'poor'::"text", 'unknown'::"text"])))
);
...
ALTER TABLE ONLY "api"."tables"
    ADD CONSTRAINT "tables_added_by_fkey" FOREIGN KEY ("added_by") REFERENCES "auth"."users"("id");
...
ALTER TABLE "api"."tables" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "API can insert tables" ON "public"."tables" FOR INSERT TO "service_role" WITH CHECK (true);



CREATE POLICY "Authenticated users can insert tables" ON "public"."tables" FOR INSERT WITH CHECK (("auth"."role"() = 'authenticated'::"text"));



CREATE POLICY "Public tables are viewable by everyone" ON "public"."tables" FOR SELECT USING (("is_public" = true));



CREATE POLICY "Users can update own tables" ON "public"."tables" FOR UPDATE USING (("auth"."uid"() = "added_by"));
...
ALTER PUBLICATION "supabase_realtime" ADD TABLE ONLY "api"."tables";
...
GRANT ALL ON FUNCTION "public"."nearby_tables"("lat" double precision, "lng" double precision, "radius_meters" double precision) TO "anon";
GRANT ALL ON FUNCTION "public"."nearby_tables"("lat" double precision, "lng" double precision, "radius_meters" double precision) TO "authenticated";
GRANT ALL ON FUNCTION "public"."nearby_tables"("lat" double precision, "lng" double precision, "radius_meters" double precision) TO "service_role";
